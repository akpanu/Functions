...
const lufthansa = {
  ...
  book(flightNum, name) {
    console.log(
      `${name} booked a seat on ${this.airline} with flight: ${this.iatacode}${flightNum}`
    );
...
// does not work cos the this keyword is referencing no object
...
console.log(`-----Demonstrating the call method-----`);
// call method uses a comma separated args list for the object method
...
console.log(`-----Demonstrating the apply method-----`);
// the apply method
...
const flightData1 = [321, `Uwem Akpan`];
...
console.log(
  `-----Demonstrating the call method using the spread (...) operator-----`
);
const flightData2 = [535, `Akpan Uwem Mfoniso`];
...
// binding the lufthansa.buyPlane method with the lufthansa object
...
console.log(addTax(0.075, 100));
// using the bind method to preset the rate
const addVAT = addTax.bind(null, 0.075);
console.log(addVAT(200));
console.log(`Amount: 400, With VAT: ${addVAT(400)}`);

// using arrow functions to implement addVAT
const addVAT2 =
  amt =>
  (r = 0.075) =>
    amt * r + amt + 1;
...
(() => console.log(`IIFE using Arrow functions - would not run again`))();
